...
export function createWorld(scene){
 ...
 const stone=new T.MeshStandardMaterial({color:0xa8b1a6,roughness:.93});
 const dark=new T.MeshStandardMaterial({color:0x18363b,roughness:.65,metalness:.32});
 ...
 const wall=new T.MeshStandardMaterial({color:0xc8c5ac,roughness:.95});
 ...
 const cyl=(r1,r2,h,m,x,y,z,p,n=24)=>mesh(new T.CylinderGeometry(r1,r2,h,n),m,x,y,z,p);
 ...
 scene.add(new T.HemisphereLight(0xb9d9ea,0x374348,2.1));const moonlight=new T.DirectionalLight(0xd1e5ed,2.5);moonlight.position.set(-25,35,-20);scene.add(moonlight);
 ...
 mesh(g,new T.MeshStandardMaterial({vertexColors:true,map:grain,roughness:1}));
 ...
 const rockGeo=new T.IcosahedronGeometry(1,1),rockMat=new T.MeshStandardMaterial({color:0x6c807b,roughness:1,flatShading:true});
 ...
 for(let i=0;i<360;i++){let z=27-rand()*97,x=center(z)+(rand()<.5?-1:1)*(3.5+rand()*21);if(z<-36&&Math.abs(x)<8)continue;let r=.25+rand()*(x>0?.95:1.9);x=center(z)+Math.sign(x-center(z))*Math.max(Math.abs(x-center(z)),halfWidth(z)+r+.55);instance('rocks',rockGeo,rockMat,x,height(x,z)-r*.25,z,r,r*(.5+rand()),r*.8,rand()*6)}
 // Individually placed paving stones remain almost flush with the continuous ground.
 const slab=new T.CylinderGeometry(1,1,.035,7),slabMat=new T.MeshStandardMaterial({color:0xa8ae9c,map:grain,roughness:1});
 ...
 // Silver coastal grass: thousands of opaque, crossed blades in one instanced draw.
 const grassGeo=new T.BufferGeometry();grassGeo.setAttribute('position',new T.Float32BufferAttribute([-.11,0,0,.07,0,0,.15,.8,.05, 0,0,-.09,0,0,.09,.05,.75,.12, -.05,.01,.04,.04,.01,-.03,-.19,.65,.06],3));grassGeo.computeVertexNormals();
 const grassMat=new T.MeshStandardMaterial({color:0x899f7b,roughness:1,side:T.DoubleSide});
 grassMat.onBeforeCompile=s=>{s.uniforms.breeze={value:0};grassMat.userData.shader=s;s.vertexShader='uniform float breeze;\n'+s.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\n transformed.x += sin(breeze+instanceMatrix[3].z*.8+instanceMatrix[3].x)*.1*position.y*position.y;')};
 for(let i=0;i<10500;i++){let z=27-rand()*97,x=center(z)+(rand()<.5?-1:1)*(2.45+rand()*16);if(z<-35&&Math.abs(x)<8)continue;const s=.35+rand()*.68;instance('grass',grassGeo,grassMat,x,height(x,z)-.025,z,s,s,s,rand()*6)}
 // Sparse cypresses, kept beyond the collision banks; layered crowns retain a coastal silhouette.
 const trunkGeo=new T.CylinderGeometry(.12,.21,1,7),trunkMat=new T.MeshStandardMaterial({color:0x4c5147,roughness:1});const crownGeo=new T.IcosahedronGeometry(1,1),leafMat=new T.MeshStandardMaterial({color:0x31534e,roughness:1,flatShading:true});
 for(let i=0;i<48;i++){const z=25-rand()*91,x=center(z)-(4.8+rand()*16);if(z<-32&&Math.abs(x)<9)continue;let h=4+rand()*5,y=height(x,z);instance('trunks',trunkGeo,trunkMat,x,y+h*.36,z,1,h*.72,1);for(let j=0;j<3;j++)instance('crowns',crownGeo,leafMat,x+.13*Math.sin(j),y+h*(.45+j*.2),z,.75-j*.17,h*.31,.65-j*.12,rand()*6)}
 // Lantern posts, warm lenses and little pools of sand-coloured light (no point-light cost).
 const postGeo=new T.CylinderGeometry(.035,.05,1,8),lensGeo=new T.CylinderGeometry(.095,.095,.26,8),capGeo=new T.ConeGeometry(.17,.13,8),poolGeo=new T.CircleGeometry(.66,20),poolMat=new T.MeshBasicMaterial({color:0xb7ad7b,transparent:true,opacity:.12,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-1});
 for(let z=18,i=0;z>-39;z-=4.4,i++){for(let side of [-1,1]){let x=center(z)+side*Math.max(2.13,halfWidth(z)+.45),y=height(x,z);instance('posts',postGeo,brass,x,y+.61,z,1,1.22,1);instance('lenses',lensGeo,warm,x,y+1.28,z,1,1,1);instance('caps',capGeo,dark,x,y+1.47,z,1,1,1);const pool=mesh(poolGeo,poolMat,x,y+.015,z);pool.rotation.x=-Math.PI/2}}
 ...
 const wallArc=mesh(new T.CylinderGeometry(5,5,3.7,64,1,true,.245,Math.PI*2-.49),wall,0,2.05,0,home);wallArc.material.side=T.DoubleSide;
 ...
 const dome=mesh(new T.SphereGeometry(5.3,48,20,0,Math.PI*2,0,Math.PI/2),dark,0,4.13,0,home);dome.scale.y=.59;
 ...
 return {door,doorPivot,slate,home,update(t,opened,dt){seaMat.uniforms.time.value=t;if(grassMat.userData.shader)grassMat.userData.shader.uniforms.breeze.value=t*.65;orrery.rotation.y=t*.045;doorPivot.rotation.y+=((opened?Math.PI*.52:0)-doorPivot.rotation.y)*(1-Math.exp(-6*dt))}};
}
